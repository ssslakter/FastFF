...
# %% ../../nbs/04_fff.ipynb 4
class FFF(nn.Module):
    def __init__(self, in_dim, out_dim, depth, act=nn.ReLU, p=0., hidden_dim = None, leak=0., save_probs=True, eval_fast=False):
        super().__init__()
        store_attr()
    ...
    def forward(self, x): 
        if not self.eval_fast: return self.train_forward(x)
        return self.train_forward(x) if self.training else self.eval_forward(x)
    ...
    def train_forward(self, x):
        ...
        if self.save_probs: self.probs = probs.detach().squeeze(-1)
        
        x = self.act(self.leaves(x)).view(bs,self.n_leaves, -1)        # (bs, n_leaves, out_dim/hidden_dim)
        if self.hidden_dim: x =  torch.stack([self.leaves_out[i](x[:,i,:]) for i in range(self.n_leaves)], dim=1) # (bs, n_leaves, out_dim)
    ...
    def eval_forward(self, x):
        ...
        if self.hidden_dim: x = torch.stack([self.leaves_out[b](x[i]) for i,b in enumerate(probs)], dim=0)
        return x
