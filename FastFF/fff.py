...
class FFF(nn.Module):
    ...
    def train_forward(self, x):
        bs = x.shape[0]
        logprobs = self.nodes(x) # (bs, n_leaves-1)
        if hasattr(self,'dropout'): logprobs = self.dropout(logprobs)
        logprobs = F.logsigmoid(torch.stack([-logprobs, logprobs],dim=2)) # (bs, n_leaves-1, 2)
